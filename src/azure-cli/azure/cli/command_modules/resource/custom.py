...
class JsonCTemplatePolicy(SansIOHTTPPolicy):
    ...
    def on_request(self, request):
        ...
        if modified_data.get('properties', {}).get('template'):
            ...
            # CRITICAL FIX: The template is currently an escaped JSON string from SDK serialization.
            # We need to parse it back to get the original JSON content to avoid size inflation.
            try:
                # Parse the escaped string back to the original JSON content
                unescaped_template = json.loads(template)
                # Convert it back to a properly formatted JSON string without unnecessary whitespace
